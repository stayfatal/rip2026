...
func UploadImage(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, strategyID uint) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "", err
	}
	...
	objectName := fmt.Sprintf("strategy_%d%s", strategyID, ext)
...
func UploadVideo(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, strategyID uint) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "", err
	}
	...
	objectName := fmt.Sprintf("strategy_%d_video%s", strategyID, ext)
